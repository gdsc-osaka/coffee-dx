...
CREATE TABLE brew_units (
  ...
  order_item_id TEXT REFERENCES order_items(id),  -- NULL = 事前仕込み（未紐付き）
  status        TEXT NOT NULL DEFAULT 'brewing'
                CHECK(status IN ('brewing', 'ready')),
  event_id      TEXT NOT NULL,  -- 業務日 YYYY-MM-DD
  created_at    TEXT NOT NULL DEFAULT (datetime('now', '+9 hours')),
  updated_at    TEXT NOT NULL DEFAULT (datetime('now', '+9 hours'))
);

CREATE INDEX idx_brew_units_menu_event  ON brew_units(menu_item_id, event_id);
...
| `menu_item_id` | どのメニューの杯か |
| `order_item_id` | どの order_item に割り当てられたか。NULL = 未紐付き（事前仕込み） |
| `status` | `brewing`（抽出中） / `ready`（完成） |
| `event_id` | 業務日。OrderDO のスコープと一致させる |
...
SNAPSHOT は接続直後に全件送信する。BrewUnit は `status = 'ready'` かつ `order_item_id IS NOT NULL` のものも含む（cashier の per-order 表示に必要）。
...
1. ドリッパーが杯数を選んで「開始」→ `POST /do/brew-units { menuItemId, count }` → BrewUnit N 件生成 → `BREW_UNITS_CREATED` broadcast → 全画面に新しいバッチカードが出現
...
3. 「余剰削除」ボタン（`order_item_id IS NULL` な杯を含むバッチのみ表示）→ `DELETE /do/brew-units/batch/:batchId` → `BREW_UNIT_DELETED` × N broadcast → 未紐付き杯のみ削除。紐付き済みの杯は残る
